...
bool fsqlUpdateImg(sqlite3* db, std::string id, int img){
...
library ReadFromDatabase(sqlite3* db){
    ...
    while (true) {
        ...
        if (stepResult == SQLITE_ROW) {
            ...
            int img = sqlite3_column_int(stmt, 5);
            ...
            book newBook(id, name, author, comment, img, rate);
